...
CMDS=(
  ...
  "docker network create golf-rental-redis || true"
  "docker ps -a --filter name=golf-rental-redis --format '{{.Names}}' | grep -q golf-rental-redis || docker run -d --name golf-rental-redis --restart=always --network golf-rental-redis -p 6379:6379 redis:7-alpine"
